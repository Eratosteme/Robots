...
--
-- Hôte : 127.0.0.1
-- Généré le : dim. 15 déc. 2024 à 18:15
...
--
-- Doublure de structure pour la vue `vue_analyse_actions`
-- (Voir ci-dessous la vue réelle)
--
CREATE TABLE `vue_analyse_actions` (
`Robot_Nom` varchar(100)
,`Nombre_Actions` bigint(21)
);

-- --------------------------------------------------------

--
-- Doublure de structure pour la vue `vue_rapport_public`
-- (Voir ci-dessous la vue réelle)
--
CREATE TABLE `vue_rapport_public` (
`ID_Rapport` int(11)
,`Type_Loi` enum('1','2','3')
,`Description_Tronquee` varchar(100)
);

-- --------------------------------------------------------

--
-- Structure de la vue `vue_analyse_actions`
--
DROP TABLE IF EXISTS `vue_analyse_actions`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vue_analyse_actions`  AS SELECT `r`.`Nom` AS `Robot_Nom`, count(`a`.`ID_Action`) AS `Nombre_Actions` FROM ((`robot` `r` left join `robot_action` `ra` on(`r`.`ID_Robot` = `ra`.`ID_Robot`)) left join `action` `a` on(`ra`.`ID_Action` = `a`.`ID_Action`)) GROUP BY `r`.`Nom` ;

-- --------------------------------------------------------

--
-- Structure de la vue `vue_rapport_public`
--
DROP TABLE IF EXISTS `vue_rapport_public`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vue_rapport_public`  AS SELECT `rapport_incidence`.`ID_Rapport` AS `ID_Rapport`, `rapport_incidence`.`Type_Loi` AS `Type_Loi`, left(`rapport_incidence`.`Description`,100) AS `Description_Tronquee` FROM `rapport_incidence` ;
